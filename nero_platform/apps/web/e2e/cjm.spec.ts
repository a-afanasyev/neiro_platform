...
test.describe('CJM #1: Родитель - Онбординг', () => {
  ...
  test('CJM #1.3: Родитель может просмотреть профиль своего ребенка', async ({ page }) => {
    ...
    // Примечание: Секция "Назначенные специалисты" отображается условно,
    // только если у ребенка есть назначенные специалисты.
    // Для базового теста онбординга важно, что профиль загрузился корректно,
    // а наличие/отсутствие специалистов не является критичной проверкой.
  })
})
