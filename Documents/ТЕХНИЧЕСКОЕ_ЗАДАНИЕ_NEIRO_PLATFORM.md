...
**Версия:** 3.5 *(обновлено 22 ноября 2025)*
...
- ✅ Синхронизированы версии артефактов: `API_CONTRACTS_MVP.md` v0.9, `DATA_MODEL_AND_EVENTS.md` v0.5, `neiro.md` v2.0.
- ✅ Таблица статусов обновлена, статусы выровнены с фактическими документами (Design System, CJM, Correction Route).
- ✅ Обновлены ссылки на актуальные версии API и модели данных в разделах интеграций и связанных артефактов.
...
- ✅ Разделы 2.1, 2.2, 9: отказ от Kafka на этапе MVP, переход на Postgres Outbox + воркеры, обновлены диаграмма и стек.
- ✅ Таблица статусов: синхронизирована версия `DATA_MODEL_AND_EVENTS.md` (0.4).
- 🔄 Следующие этапы: фокус на outbox-инфраструктуре и мониторинге вместо Kafka-кластера.

**Изменения в версии 3.2:**
- ✅ Разделы 2.1, 2.2 и 9: зафиксирована Apache Kafka как единый транспорт событий (диаграмма, стек, docker-compose, DLQ)
- 🔄 Раздел 5.4: PWA офлайн-функциональность перенесена на post-MVP (в MVP остаются manifest + push).
- ⚠️ Раздел 7.2: зафиксировано временное допущение по 152-ФЗ (локализация данных — после MVP).
- ✅ Раздел 8.2: интеграции Telegram/WebRTC/Gamification включены в MVP.
...
- ✅ По всему документу: Добавлены ссылки на `DATA_MODEL_AND_EVENTS.md` (актуальная версия 0.4) и `API_CONTRACTS_MVP.md` v0.7
...
graph TB
    ...
    subgraph "Data Layer"
        ...
        OUTBOX[(Outbox Workers<br/>Background Jobs)]
...
- Outbox-паттерн (PostgreSQL + фоновые воркеры) для событийного обмена и DLQ
...
- **Источник:** `DATA_MODEL_AND_EVENTS.md` §1.
...
- Push-уведомления
- Адаптивный дизайн
- Service Worker и офлайн кеширование — перенесены в post-MVP (см. roadmap)
...
- регламент по сборке и инструментам фронтенда - ссылка добавляется по готовности мониторинга.
...
**152-ФЗ (Россия):**
- ⚠️ Локализация данных в РФ перенесена на post-MVP; в MVP допускается временная работа в выбранном дата-центре с обязательством миграции.
...
> **📖 Полная спецификация:** `API_CONTRACTS_MVP.md` (версия 0.9)
...
- API документация (см. `API_CONTRACTS_MVP.md` v0.9)
- Комментарии в коде
- Архитектурная документация
- **✅ Модель данных:** `DATA_MODEL_AND_EVENTS.md` v0.5
...
|----------|--------|--------|----------|
| `DATA_MODEL_AND_EVENTS.md` | 0.5 | ✅ Готов | 27 таблиц с SQL DDL, 10 бизнес-правил, 36 событий; Outbox transport, добавлены события сервиса children |
| `API_CONTRACTS_MVP.md` | 0.9 | ✅ Готов | 12 сервисов, Webhook API, tRPC namespace, интеграции Telegram/WebRTC/Gamification, обновленные контракты Children/Routes |
| `DESIGN_SYSTEM.md` | 1.0.1 | ✅ Готов | Цвета, типографика, 50+ компонентов, accessibility; синхронизировано с SoT v3.5 |
| `UI_KIT.md` | 1.0 | ✅ Готов | Практические примеры, композитные паттерны |
| `Neiro_CJM_Extended.md` | 1.4 | ✅ Готов | 12 детализированных Customer Journey Maps, выровнены с SoT v3.5 и моделью данных 0.5 |
| `neiro.md` | 2.0 | ✅ Готов | Продуктовый контекст, миссия, архитектура |
