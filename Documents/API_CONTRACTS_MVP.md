...
17. [Интеграции](#17-интеграции)
   ...
   - [17.4. Offline Sync Service (Post-MVP)](#174-offline-sync-service-post-mvp)
...
- **Роль:** `specialist`, `supervisor`, `admin`.
- **Описание:** Привязка нового родителя к ребенку.
- **Запрос:**
  ```json
  {
    "parentUserId": "uuid",
  ...
  ```
- **Валидация:**
  - Пользователь `parentUserId` должен иметь роль `parent`.
  - Связь `(childId, parentUserId)` должна быть уникальной.
- **Ответ 201:** `{ "success": true, "data": { "id": "uuid", "childId": "uuid", "parentUserId": "uuid", "relationship": "...", "legalGuardian": false, "linkedAt": "..." } }`
- **События:** Публикуется `children.parent.linked`.
- **Ошибки:**
  - 400: Пользователь не имеет роли parent.
  - 409: Родитель уже привязан к ребенку.
  - 404: Ребенок или родитель не найден.

### DELETE `/children/v1/:id/parents/:parentId`
- **Роль:** `specialist`, `supervisor`, `admin`.
- **Описание:** Удаление связи родителя с ребенком.
- **Валидация:** Нельзя удалить последнего законного представителя (если `legalGuardian=true` и других опекунов нет).
- **Ответ 204:** No Content.
- **События:** Публикуется `children.parent.unlinked`.
- **Ошибки:**
  - 400: Нельзя удалить последнего законного представителя.
  - 404: Связь не найдена.

### PATCH `/children/v1/:id/parents/:parentId`
- **Роль:** `specialist`, `supervisor`, `admin`.
- **Описание:** Обновление информации о связи родителя (тип отношений, статус опекуна).
- **Запрос:**
  ```json
  {
    "relationship": "mother|father|guardian|other",
    "legalGuardian": true,
    "guardianshipType": "string<=50"
  }
  ```
- **Валидация:** Нельзя снять флаг `legalGuardian` если это последний законный представитель.
- **Ответ 200:** обновленный объект связи.
- **Ошибки:**
  - 400: Нельзя снять статус последнего опекуна.
  - 404: Связь не найдена.
...
> Все webhook-доставки формируются из Postgres outbox-воркеров (`event_outbox`) с автоматическим ретраем и переносом в `event_outbox_failures` (DLQ) при исчерпании попыток. API предоставляет инструменты для мониторинга, ручного рестарта и анализа сбоев без необходимости прямого доступа к БД.
...
- **Фильтры:** `webhookId`, `status`, `eventName`, `dateFrom`, `dateTo`, `hasFailure`.
- **Ответ 200:** пагинированный список доставок с деталями, включая ссылку на запись в DLQ (`failureRecordId`) при наличии.

### POST `/webhooks/v1/deliveries/:id/retry`
- **Роль:** `admin`.
- **Описание:** Повторная отправка webhook'а.
- **Ответ 200:** `{ "retryId": "uuid", "scheduledAt": "...", "source": "outbox" | "dlq" }`
...
> В MVP поддерживаем только базовый манифест и push-подписку. Офлайн-кеш, Service Worker и синхронизация данных перенесены в post-MVP (см. §17.4).
...
> Эти интеграции входят в MVP и реализуются одновременно с основными сервисами. Исключение: Offline Sync остаётся в статусе post-MVP (см. §17.4).
...
### 17.4 Offline Sync Service (Post-MVP)

> Статус: post-MVP. Спецификация сохраняется для планирования будущих релизов.
...
- Дополнительно к `/sync/*` эндпоинтам сохраняем спецификации PWA офлайн-слоя:
  - `GET /pwa/v1/service-worker` — выдача Service Worker с политикой кеширования.
  - `POST /pwa/v1/cache/update` — обновление кешированных ресурсов.
  - `GET /pwa/v1/offline/data` — выгрузка данных для офлайн-режима (`dataType`: `exercises|assignments|reports`).
  Эти вызовы не реализуются в MVP и остаются в backlog.
...
interface WebhookDelivery {
  ...
  failureRecordId?: string; // UUID из event_outbox_failures
...
- ✅ Документация синхронизирована с `DATA_MODEL_AND_EVENTS.md` v0.4 и SoT v3.3 (обновлены ссылки и пояснения).
- ✅ В общих сведениях уточнена дата версии и связь с предыдущими релизами.

**Changelog v0.7 (29.10.2025):**
- ✅ Перевели Telegram/WebRTC/Gamification интеграции в основной перечень MVP (обновлён §17, tRPC router).
- 🔄 PWA Service теперь покрывает только manifest + push в MVP; офлайн-кеш и sync перенесены в post-MVP (§15, §17.4).
- 🔁 Обновили tRPC namespace с активными `telegram`, `webrtc`, `gamification` роутерами.
- 🗺️ Уточнили статус offline функциональности и описали связанные эндпоинты как backlog.
